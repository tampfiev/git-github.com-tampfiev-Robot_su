...
#define MAX_DETECTION_THRESHOLD -0.1
...
void CommandDetector::run()
{
    ...
    for (int i = 0; i < COMMAND_WINDOW; i++)
    {
        for (int j = 0; j < NUMBER_COMMANDS; j++)
        {
            ...
            // Serial.printf("m_scores[%d][%d] = %.3f, ", i, j, m_scores[i][j]);
        }
    }
    // Serial.printf("\r\n");
    // get the best score
    ...
    if (best_score > DETECTION_THRESHOLD && best_index != NUMBER_COMMANDS - 1 && start - m_last_detection > 1000) //&& best_score < MAX_DETECTION_THRESHOLD
    {
